...
class TCB_Hooks {
	...
	public static function exclude_advanced_elements( $cpanel_config = array() ) {
		...
		$cpanel_config['disabled_controls']['text']['more_link']    = true;
	...
	public static function editor_layout( $current_templates, $post_id, $post_type ) {
		...
		if ( is_editor_page() && ( $post_type == 'post' || $post_type == 'page' ) ) {
		...
		if ( is_editor_page() ) {
			...
			$page_data = array(
				...
				'L'                    => array(
					...
					'missing_step_error'                  => __( 'Your page has neither a link to the next step in the quiz nor a form connected to the email service, so your visitors will be blocked on this step.', Thrive_Quiz_Builder::T ),
	...
	public static function tqb_quiz_custom_template( $single ) {
		...
		if ( $post->post_type === Thrive_Quiz_Builder::SHORTCODE_NAME ) {
	 ...
	 */
	public
	static function tqb_editor_output_custom_css(
		$variation, $return = false
	) {
	 ...
	 */
	public
	static function enable_tqb_advanced_menu(
		$post_type
	) {
